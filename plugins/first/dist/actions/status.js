...
module.exports = class Status extends actionhero_1.Action {
    constructor() {
        super();
        this.name = "status";
    ...
    async run(data) {
        data.response.uptime = new Date().getTime() - actionhero_1.api.bootTime;
        data.response.nodeStatus = data.connection.localize("Node Healthy");
        data.response.problems = [];
        data.response.id = actionhero_1.id;
        data.response.actionheroVersion = actionhero_1.actionheroVersion;
